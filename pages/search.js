import { Pagination, Rating, Box, Button, Grid, List, ListItem, MenuItem, Select, Typography } from '@mui/material';
...
export async function getServerSideProps({ query }) {
    ...
    const order = sort === 'featured' ? { featured: -1 }
        :
        sort === 'lowest' ? { price: 1 }
            :
...
const Search = (props) => {
    ...
    const { query = 'all', category = 'all', brand = 'all', price = 'all',
        rating = 'all', sort = 'featured' } = router.query;
                    ...
                    <List>
                        <ListItem>
                            <Box className={classes.fullWidth}>
                                ...
                                <Select
                                    fullWidth
                                    value={category}
                                    onChange={(e) => filterSearch({ category: e.target.value })}
                                >
                        ...
                        <ListItem>
                            <Box className={classes.fullWidth}>
                                ...
                                <Select
                                    value={brand}
                                    onChange={(e) => filterSearch({ brand: e.target.value })}
                                    fullWidth
                                >
                                    <MenuItem value="all">All</MenuItem>
                                    {brands &&
                        ...
                        <ListItem>
                            <Box className={classes.fullWidth}>
                                ...
                                <Select
                                    value={price}
                                    onChange={(e) => filterSearch({ price: e.target.value })}
                                    fullWidth
                                >
                                    <MenuItem value="all">All</MenuItem>
                                    {prices.map((price) => (
                        ...
                        <ListItem>
                            <Box className={classes.fullWidth}>
                                ...
                                <Select
                                    value={rating}
                                    onChange={(e) => filterSearch({ rating: e.target.value })}
                                    fullWidth
                                >
                                    <MenuItem value="all">All</MenuItem>
                                    {ratings.map((rating) => (
                    ...
                    <Grid container justifyContent="space-between" alignItems="center">
                        <Grid item>
                            ...
                            {(query !== 'all' && query !== '') || category !== 'all' ||
                                brand !== 'all' || rating !== 'all' || price !== 'all' ?
                        ...
                        <Grid item>
                            ...
                            <Select value={sort} onChange={(e) => filterSearch({ sort: e.target.value })}>
